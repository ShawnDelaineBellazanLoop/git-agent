"""
orchestrate.py — Orchestrator Invocation Entry Point
==================================================
Wires route_cycle.py (pure engine) + run_cycle.py (trail writer).
"""
...
def orchestrate(
    ...
    cycle.plan(steps=[
        ...
        "Apply Reflector verdict and determine phase transition"
    ])

    cycle.make(
        action=f"route(payload) -> {decision_frame['routed_to']}",
        result=decision_frame["reason"]
    )
    ...
    cycle.orchestrate_close(
        summary=f"Routed to {decision_frame['routed_to']}. Reason: {decision_frame['reason']}"
    )
